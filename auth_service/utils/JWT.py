...
from jwt import (DecodeError, ExpiredSignatureError, ImmatureSignatureError,
                 InvalidAlgorithmError, InvalidAudienceError,
...
from auth_service.utils.sql_request import add_jti_redis, get_user_id
...
async def get_refresh_jti(
    ...
    if not refresh_token:
        raise HTTPException(
            status_code=status.HTTP_401_UNAUTHORIZED, detail="Нет refresh_token"
        )
    try:
        ...
        raise HTTPException(
            status_code=400, detail=f"Отсутствует обязательное поле: {e.claim}"
        )
        ...
        raise HTTPException(
            status_code=400, detail="Неверный формат или подпись токена"
        )
    jti = payload.get("jti")
    if not jti:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="В access-токене нет jti"
        )
...
async def get_access_w_refresh(
    refresh_token: str = Cookie(None, alias="refresh_token")
):
    if not refresh_token:
        raise HTTPException(
    ...
    if user_id is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Неверный Access токен"
        )
